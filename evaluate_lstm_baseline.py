"""
Evaluation Script for LSTM Baseline Model
LSTMベースラインモデルの評価スクリプト
"""
...
def optimize_threshold(y_true, y_prob, metric='f1'):
    ...
    Args:
        y_true: 正解ラベル
        y_prob: 予測確率
        metric: 最適化するメトリクス ('f1', 'precision', 'recall')
...
def evaluate_model(
    ...
    for h in FORECAST_HORIZONS:
        ...
        results[f'{h}d'] = {
            'threshold': float(threshold),taloader, desc="Evaluating"):
    ...
    if optimize_thresholds:
        ...
        for h in FORECAST_HORIZONS:
            ...
            best_threshold, best_f1, thresholds, scores = optimize_threshold(y_true, y_prob, metric='f1')
            optimal_thresholds[h] = best_threshold
            
            print(f"\nHorizon {h} days:")
            print(f"  Optimal threshold: {best_threshold:.3f}")
            print(f"  F1 score at optimal threshold: {best_f1:.4f}")
            print(f"  F1 score at default (0.5): {f1_score(y_true, (y_prob >= 0.5).astype(int), zero_division=0):.4f}")
    else:
        optimal_thresholds = {h: 0.5 for h in FORECAST_HORIZONS}
...
def main():
    """メイン関数"""
    print("="*80)
    print("🔍 LSTM Baseline Evaluation Start")
    print("="*80)
    ...
    if not test_path.exists():
        ...
        _, test_df = train_test_split(df, test_size=0.2, random_state=42, stratify=df['label_30d'])
    else:
    ...
    summary_dThreshold': f"{h_results['threshold']:.3f}",
            'ata = []
    for h in FORECAST_HORIZONS:
    ...
    # CSV保存
    summary_csv = results_dir / f"evaluation_summary_{timestamp}.csv"
